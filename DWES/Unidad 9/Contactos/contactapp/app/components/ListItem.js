...
class ListItem extends React.Component {
  ...
  handleUpdateContact = async () => {
    const { editedName, editedPhone, editedEmail } = this.state;
    const { handleUpdate, id } = this.props;
    if (editedName && editedPhone && editedEmail) {
      await handleUpdate(editedName, editedPhone, editedEmail, id);
      this.setState({
        editMode: false,
      });
    }
  ...
  render() {
    const { nombre, telefono, email, handleDelete, id } = this.props;
